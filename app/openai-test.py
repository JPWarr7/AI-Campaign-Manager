...
def summarization(links):
    ...
    messages=[
        ...
        {"role": "user", "content": f'Give me a one-paragraph summarization for each of the following links: {links}'}
    ]
    )
...
def text_generation(summary, perspective):
    ...
    messages=[
        ...
        {"role": "user", "content": f'Compose an advertisement containing information from {summary} from the perspective of {perspective}'}
    ]
    )
    return completion.choices[0].message.content
